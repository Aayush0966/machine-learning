...
class LinearRegression:
    def best_fit(self, x, y, rg):
        ...
        if X.ndim != 2:
            raise ValueError(
                f"X should be 2-dimensional (samples, features). Got shape: {X.shape}"
            )
        if Y.ndim != 1:
            raise ValueError(
                f"Y should be 1-dimensional (samples,). Got shape: {Y.shape}"
            )
        if X.shape[0] != Y.shape[0]:
            raise ValueError(
                f"Number of samples in X and Y must match. Got X.shape[0]={X.shape[0]}, Y.shape[0]={Y.shape[0]}"
            )
        ...
        x_scaled = (
            X - x_mean
        ) / x_std  # scaling because some features are large in size and some are small
        ...
        for i in range(epochs):
            ...
            current_cost = self.cost_function(
                w_scaled, b_scaled, x_scaled, Y, alpha, rg
            )
...
class NaiveBayesClassifier:
    ...
    def calc_likelihood(self):
        ...
        for cls in self.classes:
            ...
            likelihood_cont[cls] = (1 / np.sqrt(2 * np.pi * var)) * np.exp(
                -((self.test_cont - mu) ** 2) / (2 * var)
            )
    ...
    def calc_probs(self):
        ...
        for cls in self.classes:
            self.probs[cls] = (
                self.priors[cls]
                * self.likelihood["cont"][cls]
                * self.likelihood["cat"][cls]
            )
        return self
...
class LogisticRegression:
    ...
    def cost_function(self, w, b, x, y, alpha=0, rg=None):
        ...
        cross_entropy = -(
            np.mean(y * np.log(prediction) + (1 - y) * np.log(1 - prediction))
        )
